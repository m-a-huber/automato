...
from .bottleneck_bootstrap import BottleneckBootstrap  # type: ignore
from .persistence_plotting import plot_persistences  # type: ignore
...
class Automato(_Customato, ClusterMixin, BaseEstimator):
    ...
    def fit(self, X, y=None, tomato_fit_params=dict()):
        ...
        if len(X) == 1:
            ...
            if self.create_outliers:
                ...
                X_non_outliers = X[non_outlier_ixs]
            else:
                X_non_outliers = X
            self._get_fitted_attributes(X_non_outliers)
            self.n_outliers_ = 0
            if self.create_outliers:
                self.points_ = X
    ...
    def fit_predict(self, X, y=None, tomato_fit_params=dict()):
        ...
        Returns:
            numpy.ndarray of shape (n_samples,): NumPy-array containing the
                labels assigned to the data points.
        """
        return self.fit(X, y=None, tomato_fit_params=tomato_fit_params).labels_
    ...
    def _get_fitted_attributes(self, X):
        if len(X) == 1:
